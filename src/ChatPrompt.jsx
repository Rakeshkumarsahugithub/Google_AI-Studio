...
export default function Chat() {
  ...
  return (
    <div className="min-h-screen bg-[#1e1e1e] text-white p-2 md:p-8 rounded-[2rem] mr-1 lg:mr-16 mx-2 mb-6">

      {/* Top Bar with Chat Prompt and Icons */}
      <div className="flex justify-between items-center mb-2 md:mb-4">
        <h1 className="text-base md:text-l">Chat Prompt</h1>
        <div className="flex space-x-2">
          {icons.map((item, index) => (
            <div
              key={index}
              className="relative p-1 md:p-2 rounded-full hover:bg-gray-700 cursor-pointer"
              onMouseEnter={() => setHoveredIcon(index)}
              onMouseLeave={() => setHoveredIcon(null)}
            >
              <div className="text-gray-400">{item.icon}</div>
      ...
      <div className="border-b border-gray-600 mb-4 md:mb-8"></div>

      {/* Main Centered Content */}
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-180px)]">
        <h2 className="text-xl md:text-4xl font-semibold text-center bg-clip-text text-transparent bg-gradient-to-r from-blue-100 via-blue-500 to-blue-900 mb-4 md:mb-10">
          Welcome to AI Studio
        </h2>

        {/* Input Box */}
        <div className="flex items-center bg-[#2a2a2a] rounded-full px-2 py-2 md:px-4 md:py-3 mb-4 md:mb-8 w-full max-w-xs md:max-w-3xl relative">
          <input
            value={prompt}
            onChange={handleInputChange}
            placeholder={sentences[index]}
            className="flex-1 bg-transparent text-xs md:text-sm px-2 md:px-4 py-1 md:py-2 focus:outline-none rounded-full text-gray-300 placeholder-gray-400"
          />

          {/* File Upload */}
          <div className="relative group mr-1 md:mr-2">
            <label htmlFor="file-upload" className="cursor-pointer text-gray-400 hover:text-white">
              <div className="w-5 h-5 md:w-6 md:h-6 flex items-center justify-center rounded-full border border-gray-500 group-hover:border-white">
          ...
          {/* Run Button */}
          <div className="relative group">
            <button className="flex items-center gap-1 bg-[#3c3c3c] hover:bg-[#505050] text-xs md:text-sm px-2 md:px-4 py-1 rounded-full text-gray-300">
        ...
        {/* What's New Section */}
        <div className="w-full max-w-xs md:max-w-5xl px-1 md:px-2">
          <h3 className="text-base md:text-l font-semibold mb-2 md:mb-6">What's new</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-2 md:gap-4">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="relative bg-[#2d2d2d] p-2 md:p-4 rounded-2xl flex flex-col justify-between min-h-[60px] md:min-h-[100px] hover:bg-[#353535] transition"
            >
              <span className="absolute top-2 right-2 bg-[#98c0e6] text-blue-900 text-[11px] px-1 py-[2px] rounded-full font-medium">
                New
              </span>
              <div className="flex items-start gap-3">
                <img src={NativeSpeech} alt="Speech" className="w-8 h-8 md:w-12 md:h-12 rounded-lg object-cover" />
                <div>
                  <p className="text-xs md:text-sm font-semibold text-gray-200">Native speech generation</p>
            ...
            >
              ...
              <div className="flex items-start gap-3">
                <img src={LiveAudio} alt="Audio" className="w-8 h-8 md:w-12 md:h-12 rounded-lg object-cover" />
                <div>
                  <p className="text-xs md:text-sm font-semibold text-gray-200">Live audio-to-audio dialog</p>
                  <p className="text-xs md:text-sm text-gray-400 mt-1">
                    Try Gemini's natural, real-time dialog with audio and video inputs
                  </p>
                </div>
            ...
            <Link
              to="/stream"
              className="bg-[#2d2d2d] p-2 md:p-4 rounded-2xl flex flex-col justify-between min-h-[60px] md:min-h-[100px] hover:bg-[#353535] transition"
            >
              <div className="flex items-start gap-3">
                <img src={NativeImage} alt="Image" className="w-8 h-8 md:w-12 md:h-12 rounded-lg object-cover" />
                <div>
                  <p className="text-xs md:text-sm font-semibold text-gray-200">Native image generation</p>
                  <p className="text-xs md:text-sm text-gray-400 mt-1">
                    Interleaved text-and-image generation with the new Gemini 2.0 Flash
                  </p>
                </div>
            ...
            >
              <div className="flex items-start gap-3">
                <img src={Explore} alt="App" className="w-8 h-8 md:w-12 md:h-12 rounded-lg object-cover" />
                <div>
                  <p className="text-xs md:text-sm font-semibold text-gray-200">Explore and co-develop apps</p>
                  <p className="text-xs md:text-sm text-gray-400 mt-1">
                    See Gemini in action with interactive, open source examples
                  </p>
                </div>
